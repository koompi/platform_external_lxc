...
static char *mk_devtmpfs(const char *name, char *path, const char *lxcpath)
{
	 ...
	 */
	if (!mount_check_fs("/dev", fstype) || strcmp("devtmpfs", fstype))
	{
									  ...
									  const struct lxc_console *console)
{
	...
	// pionux: dropping safe_mount b/c it causes problems on some Android kernels
...
static int setup_netdev(struct lxc_netdev *netdev)
{
	...
	if (!netdev->name)
		netdev->name = netdev->type == LXC_NET_PHYS ? netdev->link : "eth%d";
